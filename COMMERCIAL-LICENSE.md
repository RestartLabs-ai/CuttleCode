# Commercial License

CuttleCode is dual-licensed:

- **Noncommercial use** is governed by [LICENSE.md](./LICENSE.md) (PolyForm
  Noncommercial 1.0.0) and is free.
- **Commercial use requires a separate commercial license** from Stratus Software Solutions.

## When you need a commercial license

You need a commercial license for any use that is **not** a permitted
noncommercial purpose under [LICENSE.md](./LICENSE.md). In particular, commercial
use includes:

- Use by or for a for-profit organization, of any size.
- Use in the course of employment, contracting, or consulting — i.e. using CuttleCode
  "at work" or to deliver work for a client.
- Use in internal business tooling, operations, or any revenue-generating
  activity.

Personal, hobby, and amateur use, and use by charitable organizations,
educational institutions, public research organizations, public safety or health
organizations, environmental protection organizations, and government
institutions, remain free under [LICENSE.md](./LICENSE.md) and do **not** require
a commercial license.

If you are unsure whether your use is commercial, assume it is and contact us.

## How to obtain one

Contact Stratus Software Solutions to purchase a commercial license:

- **Email:** _<add commercial-licensing contact email>_
- **Web:** _<add purchasing/contact URL>_

A signed commercial license agreement governs your use of CuttleCode **in place of**
[LICENSE.md](./LICENSE.md). The terms, pricing, and scope (such as seat or host
counts) are set out in that agreement, not in this repository.
